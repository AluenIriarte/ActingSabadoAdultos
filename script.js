...
function loadFromLocalStorage() {
    ...
    // =====================================
// 1️⃣ Reiniciar todas las asignaciones
// =====================================
...
// =====================================
// 2️⃣ Editar / eliminar actores
// =====================================
function editActors() {
    const actorsList = document.getElementById('actorsList');
    const actorCards = Array.from(actorsList.querySelectorAll('.actor-card'));

    actorCards.forEach(actor => {
        const actorId = actor.dataset.actorId;

        // Crear botón de eliminar si no existe
        if (!actor.querySelector('.edit-remove-btn')) {
            const btn = document.createElement('button');
            btn.textContent = '🗑';
            btn.className = 'edit-remove-btn';
            btn.style.marginLeft = 'auto';
            btn.style.cursor = 'pointer';
            btn.title = 'Eliminar actor';
            btn.onclick = (e) => {
                e.stopPropagation();
                // Remover actor de todas las asignaciones primero
                removeActorFromAllPlays(actorId);
                actor.remove();
                saveToLocalStorage();
                updateStats();
            };
            actor.appendChild(btn);
        }
    });

    alert('Botones de eliminar añadidos a cada actor. Haz clic en 🗑 para eliminar.');
}

// =====================================
// 3️⃣ Editar / eliminar obras
// =====================================
function editPlays() {
    const plays = Array.from(document.querySelectorAll('.play-card'));

    plays.forEach(play => {
        const playId = play.querySelector('.drop-zone').dataset.playId;

        // Crear botón de eliminar si no existe
        if (!play.querySelector('.edit-remove-play')) {
            const btn = document.createElement('button');
            btn.textContent = '🗑';
            btn.className = 'edit-remove-play';
            btn.style.marginLeft = 'auto';
            btn.style.cursor = 'pointer';
            btn.title = 'Eliminar obra';
            btn.onclick = (e) => {
                e.stopPropagation();
                // Primero remover todos los actores asignados a esta obra
            ...
            // Agregar al header de la obra
            play.querySelector('.play-header').appendChild(btn);
        }
    });

    alert('Botones de eliminar añadidos a cada obra. Haz clic en 🗑 para eliminar.');
}
